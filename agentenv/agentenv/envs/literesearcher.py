...
LITERESEARCHER_CHECKPOINT_CONTENT_SCHEMA = (
    "agentmemory_literesearcher_relation_checkpoint_v1"
)
...
LITERESEARCHER_CONTEXT_COMPACTION_EXAMPLE = _render_workspace_tool_call(
    ...
    f"schema: {LITERESEARCHER_CHECKPOINT_CONTENT_SCHEMA}\n"
    "question: ...\n"
    "candidate_answer: ...\n"
    "supported_relations:\n"
    "- relation: ...\n"
...
LITERESEARCHER_CONTEXT_COMPACTION_REQUEST = (
    ...
    "write also works in an otherwise empty workspace. Record one current "
    "candidate_answer, then record each established question relation under "
    "supported_relations with a fact and exact source_url; repeat relation entries when "
    "multiple facts are needed. For every established relation, copy its exact "
    "source-backed name, number, date, title, or relation value verbatim into fact, "
    "even when the same value appears in candidate_answer. Never replace it with a "
    "generic label or omit it from supported_relations. Unsupported guesses must not "
    "be recorded as facts. A candidate mention alone is not support. List every missing "
    "decisive relation under unresolved_relations. "
    "If none remain, write unresolved_relations: [] and set next_action to answer. If "
    "space is tight, drop low-value history before any supported or unresolved relation. "
    "This is a write-only phase. "
...
LITERESEARCHER_POLICY_CONTINUATION_MARKER = (
    ...
    "Treat the file as your own unverified notes. After reading it, answer directly if "
    "its source-backed facts support every decisive relation; otherwise investigate one "
    "listed unresolved relation. Do not copy it into another note or perform peripheral "
    "verification. The complete executable action is the final line below:\n"
        ...
        ConversationMessage(
            {
                ...
                "value": (
                    ...
                    "Search instead. If a URL is rejected as outside the released corpus, "
                    "never retry that URL or a modified variant; Search for another returned "
                    "URL instead. A Visit returns one bounded page; follow next_page "
                    "with the same URL and goal when needed. For the current unresolved relation, "
                    "if Search results visibly match the needed entity, date, title, quote, or "
                    "relation, Visit the strongest match. If none matches or Visit fails, "
                    "reformulate that relation's query. Answer from Visit evidence, not result "
                    "titles, as soon as sufficient; do not exhaust other relations or results. "
                    "A workspace turn uses function "
                    ...
                    "it only when evidence with source URLs, failed attempts, or a plan must "
                    "survive several later actions or a future context checkpoint. Do not write "
                    "it after every useful Visit. One optional penultimate synthesis write is "
                    "allowed when it helps construct the final answer and enough action budget "
                    "remains. It is never required or rewarded; direct answering remains valid. "
                    "To create or replace the note, copy this action "
