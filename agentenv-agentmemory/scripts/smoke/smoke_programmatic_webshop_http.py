...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--surface",
        choices=(LATENT_SURFACE, PROCEDURAL_SURFACE),
        required=True,
    )
...
def main() -> None:
    args = parse_args()
    data_indices = args.data_indices or ([0, 1] if args.surface == LATENT_SURFACE else [0])
    ...
    if generator_seed is None:
        generator_seed = 233 if args.surface == LATENT_SURFACE else 0
