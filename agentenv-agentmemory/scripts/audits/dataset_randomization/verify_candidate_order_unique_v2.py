#!/usr/bin/env python3
from __future__ import annotations

import argparse
import hashlib
import json
import math
from collections import Counter, defaultdict
from pathlib import Path
from typing import Any

from agentenv_agentmemory.presentation_randomization import (
    PRESENTATION_LABEL_CONTRACT,
    PRESENTATION_RANDOMIZATION_CANDIDATE_ORDER_UNIQUE_V2,
    reorder_candidate_options,
    split_candidate_block,
    stable_unique_candidate_permutation,
    unique_bundle_permutation_ranks,
)


SCHEMA = "memoryarena_candidate_order_unique_audit_v2"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Prove that seeded MemoryArena bundle variants preserve every frozen "
            "field, are unique as six-session tuples, and maximize per-session "
            "permutation coverage."
        )
    )
    parser.add_argument("--raw-data", type=Path, required=True)
    parser.add_argument("--base-seed", type=int, required=True)
    parser.add_argument("--replicas", type=int, default=8)
    parser.add_argument("--output", type=Path, required=True)
    return parser


def main() -> int:
    args = build_parser().parse_args()
    if args.replicas < 1:
        raise SystemExit("--replicas must be positive")
    payload = args.raw_data.read_bytes()
    rows = _decode_jsonl(payload)
    source_answers_sha256 = _answers_sha256(rows)
    identity_count = 0
    variant_session_count = 0
    option_count_by_session: dict[int, Counter[int]] = defaultdict(Counter)
    unique_count_by_session: dict[int, Counter[int]] = defaultdict(Counter)
    unique_bundle_count_by_parent: Counter[int] = Counter()
    records: list[dict[str, Any]] = []

    for source_position, row in enumerate(rows):
        _validate_row(row, source_position)
        task_id = row["category"]
        source_row_id = row["id"]
        source_blocks = [
            split_candidate_block(question) for question in row["questions"]
        ]
        option_counts = tuple(len(block.option_lines) for block in source_blocks)
        session_permutations: list[set[tuple[int, ...]]] = [
            set() for _ in source_blocks
        ]
        bundle_signatures: set[tuple[tuple[int, ...], ...]] = set()

        for replica in range(args.replicas):
            ranks = unique_bundle_permutation_ranks(
                option_counts=option_counts,
                variant_index=replica,
            )
            rendered_hashes: list[str] = []
            source_hashes: list[str] = []
            permutations: list[tuple[int, ...]] = []
            for session_index, (question, source, rank) in enumerate(
                zip(row["questions"], source_blocks, ranks)
            ):
                permutation = stable_unique_candidate_permutation(
                    base_seed=args.base_seed,
                    task_id=task_id,
                    source_row_id=source_row_id,
                    session_index=session_index,
                    option_titles=source.option_titles,
                    permutation_rank=rank,
                )
                rendered = reorder_candidate_options(question, permutation)
                transformed = split_candidate_block(rendered)
                if transformed.prefix != source.prefix or transformed.suffix != source.suffix:
                    raise SystemExit(
                        f"non-candidate text changed at row={source_position} "
                        f"session={session_index} replica={replica}"
                    )
                if transformed.option_endings != source.option_endings:
                    raise SystemExit(
                        f"candidate row boundaries changed at row={source_position} "
                        f"session={session_index} replica={replica}"
                    )
                if sorted(transformed.option_lines) != sorted(source.option_lines):
                    raise SystemExit(
                        f"candidate content changed at row={source_position} "
                        f"session={session_index} replica={replica}"
                    )
                session_permutations[session_index].add(permutation)
                identity_count += permutation == tuple(range(len(source.option_lines)))
                variant_session_count += 1
                permutations.append(permutation)
                source_hashes.append(_sha256_text(question))
                rendered_hashes.append(_sha256_text(rendered))

            signature = tuple(permutations)
            if signature in bundle_signatures:
                raise SystemExit(
                    f"duplicate six-session variant at row={source_position} "
                    f"replica={replica}"
                )
            bundle_signatures.add(signature)
            records.append(
                {
                    "source_position": source_position,
                    "source_row_id": source_row_id,
                    "task_id": task_id,
                    "replica": replica,
                    "variant_index": replica,
                    "permutation_ranks": list(ranks),
                    "candidate_permutations": [list(value) for value in permutations],
                    "source_question_sha256s": source_hashes,
                    "rendered_question_sha256s": rendered_hashes,
                    "bundle_variant_sha256": _sha256_json(
                        [list(value) for value in permutations]
                    ),
                }
            )

        if len(bundle_signatures) != args.replicas:
            raise SystemExit(
                f"row={source_position} has {len(bundle_signatures)} unique bundle "
                f"variants for {args.replicas} replicas"
            )
        unique_bundle_count_by_parent[len(bundle_signatures)] += 1
        for session_index, (option_count, observed) in enumerate(
            zip(option_counts, session_permutations), start=1
        ):
            expected = min(args.replicas, math.factorial(option_count))
            if len(observed) != expected:
                raise SystemExit(
                    f"row={source_position} session={session_index} has "
                    f"{len(observed)} unique permutations; expected {expected}"
                )
            option_count_by_session[session_index][option_count] += 1
            unique_count_by_session[session_index][len(observed)] += 1

    if _answers_sha256(rows) != source_answers_sha256:
        raise SystemExit("frozen answers changed during presentation audit")
    output = {
        "schema": SCHEMA,
        "status": "pass",
        "mode": PRESENTATION_RANDOMIZATION_CANDIDATE_ORDER_UNIQUE_V2,
        "label_contract": PRESENTATION_LABEL_CONTRACT,
        "source": {
            "path": str(args.raw_data.resolve()),
            "sha256": hashlib.sha256(payload).hexdigest(),
            "answer_payload_sha256": source_answers_sha256,
            "row_count": len(rows),
            "session_count": sum(len(row["questions"]) for row in rows),
        },
        "configuration": {
            "base_seed": args.base_seed,
            "replicas": args.replicas,
        },
        "proof": {
            "changes_target_asins": False,
            "changes_answer_attributes": False,
            "changes_non_candidate_text": False,
            "changes_candidate_text": False,
            "changes_candidate_order": True,
            "derived_bundle_instance_count": len(rows) * args.replicas,
            "variant_session_instance_count": variant_session_count,
            "identity_permutation_count": identity_count,
            "non_identity_permutation_count": variant_session_count - identity_count,
            "all_bundle_variants_unique_within_parent": True,
            "unique_bundle_variant_count_by_parent": _counter_json(
                unique_bundle_count_by_parent
            ),
            "option_count_by_session": _nested_counter_json(option_count_by_session),
            "unique_permutation_count_by_session_and_parent": _nested_counter_json(
                unique_count_by_session
            ),
        },
        "records": records,
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(
        json.dumps(output, ensure_ascii=True, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print(
        json.dumps(
            {
                "status": "pass",
                "rows": len(rows),
                "bundle_instances": len(rows) * args.replicas,
                "variant_session_instances": variant_session_count,
                "non_identity": variant_session_count - identity_count,
                "output": str(args.output),
            },
            sort_keys=True,
        )
    )
    return 0


def _decode_jsonl(payload: bytes) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for line_number, line in enumerate(payload.decode("utf-8").splitlines(), start=1):
        if not line.strip():
            raise SystemExit(f"blank JSONL line {line_number}")
        row = json.loads(line)
        if not isinstance(row, dict):
            raise SystemExit(f"JSONL line {line_number} is not an object")
        rows.append(row)
    if not rows:
        raise SystemExit("raw dataset is empty")
    return rows


def _validate_row(row: dict[str, Any], source_position: int) -> None:
    if row.get("id") != source_position:
        raise SystemExit(
            f"source identity mismatch at position {source_position}: {row.get('id')!r}"
        )
    if not isinstance(row.get("category"), str) or not row["category"]:
        raise SystemExit(f"invalid category at row {source_position}")
    questions = row.get("questions")
    answers = row.get("answers")
    if not isinstance(questions, list) or len(questions) != 6:
        raise SystemExit(f"row {source_position} must have six questions")
    if not isinstance(answers, list) or len(answers) != 6:
        raise SystemExit(f"row {source_position} must have six answers")
    for session_index, question in enumerate(questions):
        if not isinstance(question, str):
            raise SystemExit(
                f"row {source_position} session {session_index} question is not text"
            )
        split_candidate_block(question)


def _answers_sha256(rows: list[dict[str, Any]]) -> str:
    payload = json.dumps(
        [row.get("answers") for row in rows],
        ensure_ascii=True,
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()


def _sha256_text(value: str) -> str:
    return hashlib.sha256(value.encode("utf-8")).hexdigest()


def _sha256_json(value: Any) -> str:
    payload = json.dumps(
        value,
        ensure_ascii=True,
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()


def _counter_json(counter: Counter[int]) -> dict[str, int]:
    return {str(key): counter[key] for key in sorted(counter)}


def _nested_counter_json(
    counters: dict[int, Counter[int]],
) -> dict[str, dict[str, int]]:
    return {
        str(key): _counter_json(counters[key])
        for key in sorted(counters)
    }


if __name__ == "__main__":
    raise SystemExit(main())
