#!/usr/bin/env python3
"""Generate and exhaustively verify a hidden-preference task window."""
...
from agentenv_agentmemory.latent_preference.verifier import VERIFIER_VERSION
...
def main() -> None:
    ...
    if args.task_count <= 0 or args.task_count % 2:
        raise LatentPreferenceDataError(
            "--task-count must be a positive even integer."
        )
    if args.start_orbit < 0:
        raise LatentPreferenceDataError("--start-orbit must be non-negative.")
    ...
    evidence_orbit_counts: Counter[int] = Counter()
    first_task_id = None
    last_task_id = None
    total_certified_candidate_checks = 0
    total_evidence_target_flips = 0
    total_application_target_flips = 0
    total_application_identity_checks = 0
    ...
    for offset in range(orbit_count):
        ...
        if orbit.orbit_id in orbit_ids:
            raise LatentPreferenceDataError(f"duplicate orbit ID: {orbit.orbit_id}")
        ...
        evidence_orbit_counts[proof.supporting_evidence_count] += 1
        ...
        total_evidence_target_flips += proof.evidence_target_flip_count
        total_application_target_flips += proof.application_target_flip_count
        total_application_identity_checks += (
    ...
    if not used_asins <= split_pool_asins:
        raise LatentPreferenceDataError(
            "generated window used a product from another split."
        )
    manifest = {
        "schema": "agentmemory_verified_latent_preference_dataset_window_v1",
        "product_pool": {
        ...
        "coverage": {
            ...
            "supporting_evidence_orbit_counts": {
                str(key): value
                for key, value in sorted(evidence_orbit_counts.items())
            },
        ...
        "verification": {
            ...
            "preference_axis_count_per_task": 1,
            "resolution_step": 1,
            "supporting_evidence_counts": [1, 2, 3],
            "evidence_target_flip_checks": total_evidence_target_flips,
            "application_target_flip_checks": total_application_target_flips,
            "application_observation_identity_checks": (
                total_application_identity_checks
            ),
            "certified_candidate_checks": total_certified_candidate_checks,
            "all_purchase_vectors_within_budget": True,
    ...
    print(
        "AGENTMEMORY_LATENT_PREFERENCE_DATASET_VERIFIED "
