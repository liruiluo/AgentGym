from __future__ import annotations

from .env_wrapper import LATENT_PREFERENCE_PROMPT_MODE
from .negative_constraint import (
    PROVIDER_MODE_FIXED_WINDOW,
    PROVIDER_MODES,
    NegativeConstraintGenerator,
    VerifiedNegativeConstraintBundleProvider,
    load_negative_constraint_native_product_pool,
)
from .negative_constraint.runtime_attestation import (
    attest_negative_constraint_runtime_inputs,
)
from .negative_constraint_webshop_env import (
    NEGATIVE_CONSTRAINT_SURFACE,
    NegativeConstraintWebShopEnv,
)
from .procedural_wrapper import (
    ProceduralAgentMemoryWrapper,
    _env_choice,
    _env_int,
    _programmatic_runtime_inputs,
    _required_env,
    _required_file,
    _required_int,
)


class NegativeConstraintAgentMemoryWrapper(ProceduralAgentMemoryWrapper):
    """AgentGym HTTP wrapper for certified exclusion-constraint training."""

    surface = NEGATIVE_CONSTRAINT_SURFACE
    environment_type = NegativeConstraintWebShopEnv

    def __init__(self) -> None:
        self._initialize_native_training_runtime(
            forbidden_env_keys=(
                "AGENTMEMORY_MEMORYARENA_RAW_PATH",
                "AGENTMEMORY_ANNOTATION_GATE_MANIFEST",
                "AGENTMEMORY_ANNOTATION_MANUAL_EVIDENCE",
                "AGENTMEMORY_PROCEDURAL_PRODUCT_POOL",
                "AGENTMEMORY_PROCEDURAL_PRODUCT_POOL_SHA256",
                "AGENTMEMORY_LATENT_PREFERENCE_PRODUCT_POOL",
                "AGENTMEMORY_LATENT_PREFERENCE_PRODUCT_POOL_SHA256",
                "AGENTMEMORY_RECENCY_OVERRIDE_PRODUCT_POOL",
                "AGENTMEMORY_RECENCY_OVERRIDE_PRODUCT_POOL_SHA256",
                "AGENTMEMORY_DISTRACTOR_ROBUSTNESS_PRODUCT_POOL",
                "AGENTMEMORY_DISTRACTOR_ROBUSTNESS_PRODUCT_POOL_SHA256",
                "AGENTMEMORY_COMPOSITIONAL_RECALL_PRODUCT_POOL",
                "AGENTMEMORY_COMPOSITIONAL_RECALL_PRODUCT_POOL_SHA256",
                "AGENTMEMORY_INTENT_CLARIFICATION_PRODUCT_POOL",
                "AGENTMEMORY_INTENT_CLARIFICATION_PRODUCT_POOL_SHA256",
                "AGENTMEMORY_SELECTIVE_MEMORY_USE_PRODUCT_POOL",
                "AGENTMEMORY_SELECTIVE_MEMORY_USE_PRODUCT_POOL_SHA256",
            )
        )
        if self.memory_prompt_mode != LATENT_PREFERENCE_PROMPT_MODE:
            raise RuntimeError(
                "The negative-constraint surface requires "
                f"memory_prompt_mode={LATENT_PREFERENCE_PROMPT_MODE!r}."
            )
        pool_file_sha256 = _required_env(
            "AGENTMEMORY_NEGATIVE_CONSTRAINT_PRODUCT_POOL_SHA256"
        )
        pool = load_negative_constraint_native_product_pool(
            _required_file("AGENTMEMORY_NEGATIVE_CONSTRAINT_PRODUCT_POOL"),
            expected_file_sha256=pool_file_sha256,
        )
        self.runtime_inputs = _programmatic_runtime_inputs(
            pool,
            product_pool_file_sha256=pool_file_sha256,
        )
        attest_negative_constraint_runtime_inputs(
            pool,
            self.backend,
            items_file=self.items_file,
            attributes_file=self.attributes_file,
            search_root=self.search_root,
            lucene_manifest=self.lucene_manifest,
        )
        self.provider = VerifiedNegativeConstraintBundleProvider(
            generator=NegativeConstraintGenerator(
                pool=pool,
                seed=_required_int("AGENTMEMORY_NEGATIVE_CONSTRAINT_GENERATOR_SEED"),
            ),
            split=_required_env("AGENTMEMORY_SPLIT"),
            task_count=_required_int("AGENTMEMORY_NEGATIVE_CONSTRAINT_TASK_COUNT"),
            mode=_env_choice(
                "AGENTMEMORY_NEGATIVE_CONSTRAINT_PROVIDER_MODE",
                default=PROVIDER_MODE_FIXED_WINDOW,
                choices=PROVIDER_MODES,
            ),
            start_orbit=_env_int("AGENTMEMORY_NEGATIVE_CONSTRAINT_START_ORBIT", 0),
        )
        self._initialize_wrapper_state()
