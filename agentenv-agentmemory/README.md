...
`session_handoff` output is not trusted merely because it was generated by the
policy. Runtime must accept only one workspace-relative path, one read-only
workspace discovery/read command, or an explicit no-locator result. It must
fail closed on absolute/host paths, `file://`, traversal, prose summaries,
multiline or multi-item content, write commands, and shell operators. Invalid
raw output remains part of the sampled PPO action and audit trail but must not
be forwarded to the next session. A runtime test must prove both valid
forwarding and invalid-not-forwarded behavior.
