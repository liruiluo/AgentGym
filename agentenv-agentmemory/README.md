...
A policy-authored session handoff is one ordinary executable workspace action.
After a correct `click[Buy Now]` advances the native task, the next policy turn
must create or overwrite `.agent_memory/CONTINUATION.md` with a non-empty,
at-most-8192-byte continuation snapshot. The action is sent through the same
native `env.step` path, consumes the global policy-action budget, and keeps its
sampled tokens/logprobs in the PPO ledger. The client clears the old dialogue
only after the endpoint receipt proves that this exact action changed the fixed
regular file. A failed or unrelated action leaves the old context intact and
requests a retry.

The naming is normative: `session_reset` means that the native WebShop server
started the next shopping session; `session_handoff` means the policy-authored
filesystem checkpoint at that boundary; `context_compaction` means the same
filesystem-checkpoint mechanism inside an otherwise continuous episode. The
serialized wrapper events remain `webshop_session_handoff` and
`context_compaction` so the lifecycle distinction stays observable.

The replacement context contains only immutable task framing, the fresh native
session observation, the fixed continuation path, size, digest, and an
instruction to read it. It never injects the checkpoint action, its observation,
or the file body. The next policy turn must use a normal workspace action to read
the file. This preserves a learnable write/read chain and prevents the harness
from turning a write into a free hidden read. Other workspace files remain
available for ordinary task artifacts, logs, and longer-lived evidence.
